--[[
    Once the train has started entering the tunnel and is committed (first carriage removed from aboveground), remove a piece of track on the exit side of the tunnel. An alternative path by reversing through the tunnel exists.
]]
...
Test.Start = function(TestManager, testName)
    ...
    EventScheduler.RegisterScheduledEventType("Test.forceRepathBackThroughTunnelOnTick", Test.forceRepathBackThroughTunnelOnTick)
    EventScheduler.ScheduleEventEachTick("Test.forceRepathBackThroughTunnelOnTick", train.id, {trainId = train.id, train = train, trackToRemove = trackToRemove})
end

Test.forceRepathBackThroughTunnelOnTick = function(event)
    if event.data.train.valid then
        return
    end

    -- Train carriage has been removed as Lue Train object no longer is valid.
    EventScheduler.RemoveScheduledEventFromEachTick("Test.forceRepathBackThroughTunnelOnTick", event.data.trainId)
