...
- Event Attributes:
    ...
    - changeReason: the cause of the change (STRING):
        ...
        - abortedApproach: Raised for "terminated" and "onPortalTrack" action.
            ...
            - onPortalTrack action has this change reason when a train has been approaching the tunnel and has passed on to the portal's rail tracks, but then aborts its approach. The train remains in the "onPortalTrack" state until it has either left the portal's rail tracks, resumes its approach to the tunnel or enters the tunnel. So the change reason is reporting a downgrade in trains usage of the tunnel and thus how its monitored.
