...
TrainManager.CreateGlobals = function()
    ...
    --[[
        [id] = {
            ...
            enteringTrainLeadCarriageCache = A table to cache the lead carriage of the entering train. Has the value of 'enteringTrainForwards' when cached and 'carriage' as a ref to the lead carriage entity. Is only used and updated during TrainManager.TrainEnteringOngoing() and TrainManager.TrainApproachingOngoing().
            ...
            undergroundTrainCarriageCount = Cache of the total underground train carriage count.
            undergroundTrainLeadCarriageCache = A table to cache the lead carriage of the underground train. Has the value of 'undergroundTrainForwards' when cached and 'carriage' as a ref to the lead carriage entity. Is only used and updated during TrainManager.TrainUndergroundOngoing().
            undergroundTrainOldAbsoluteSpeed = the absolute speed of the underground train last tick. Updated once enteringStarted up untill fullLeft.
            undergroundTrainAForwardsLocoCache = a loco facing forwards in the underground train. Populated if the train runs out of fuel, not updated apart from a reversal clears it.
            undergroundTrainAForwardsLocoBurnerCache = the cached loco facing forwards burner in the underground train. Populated if the train runs out of fuel, not updated apart from a reversal clears it.
            ...
            leavingTrainRearCarriageCache = A table to cache the rear carriage of the leaving train. Has 'speedPositive' BOOLEAN and 'carriage' as a ref to the rear carriage entity. Is only used and updated during TrainManager.TrainLeavingOngoing().
...
TrainManager.TrainApproachingOngoing = function(trainManagerEntry)
    ...
    local undergroundTrainSpeed = trainManagerEntry.undergroundTrain.speed
    -- trainManagerEntry.enteringTrainForwards is updated by SetAbsoluteTrainSpeed().
    TrainManager.SetAbsoluteTrainSpeed(trainManagerEntry, "enteringTrain", math.abs(undergroundTrainSpeed))
...
TrainManager.TrainEnteringOngoing = function(trainManagerEntry)
    ...
    if trainManagerEntry.leavingTrainState == LeavingTrainStates.pre then
        TrainManager.UpdatePortalExitSignalPerTick(trainManagerEntry)
        TrainManager.EnsureManagedTrainsFuel(trainManagerEntry, math.abs(undergroundTrainSpeed))
    end
    ...
    if undergroundTrainSpeed ~= 0 and nextCarriage.position[trainManagerEntry.tunnel.tunnelAlignmentAxis] == trainManagerEntry.aboveEntrancePortal.entity.position[trainManagerEntry.tunnel.tunnelAlignmentAxis] and Utils.GetDistanceSingleAxis(nextCarriage.position, trainManagerEntry.aboveEntrancePortalEndSignal.entity.position, trainManagerEntry.tunnel.railAlignmentAxis) < 14 then
        ...
        if enteringTrain.valid then
            TrainManager.GetEnteringTrainLeadCarriageCache(trainManagerEntry, enteringTrain, nil)
        end
...
TrainManager.TrainUndergroundOngoing = function(trainManagerEntry)
    ...
    if trainManagerEntry.enteringTrainState == EnteringTrainStates.finished then
        TrainManager.UpdatePortalExitSignalPerTick(trainManagerEntry)
        TrainManager.EnsureManagedTrainsFuel(trainManagerEntry, math.abs(trainManagerEntry.undergroundTrain.speed))
    end
    ...
    local leadCarriage
    if trainManagerEntry.undergroundTrainLeadCarriageCache == nil or trainManagerEntry.undergroundTrainLeadCarriageCache.undergroundTrainForwards ~= trainManagerEntry.undergroundTrainForwards then
        -- No cache entry or cache exists, but needs updating.
        leadCarriage = TrainManagerFuncs.GetLeadingWagonOfTrain(trainManagerEntry.undergroundTrain, trainManagerEntry.undergroundTrainForwards)
        trainManagerEntry.undergroundTrainLeadCarriageCache = {
            undergroundTrainForwards = trainManagerEntry.undergroundTrainForwards,
            carriage = leadCarriage
        }
    else
        -- Use the cache lead carriage.
        leadCarriage = trainManagerEntry.undergroundTrainLeadCarriageCache.carriage
    end
...
TrainManager.TrainLeavingFirstCarriage = function(trainManagerEntry)
    ...
    TrainManagerFuncs.TrainSetSchedule(trainManagerEntry.leavingTrain, schedule, isManual, targetTrainStop)
    ...
    if trainManagerEntry.leavingTrainCarriagesPlaced == trainManagerEntry.undergroundTrainCarriageCount then
...
TrainManager.TrainLeavingOngoing = function(trainManagerEntry)
    ...
    if desiredSpeed ~= 0 and leavingTrainSpeed ~= 0 then
        ...
        if trainManagerEntry.leavingTrainRearCarriageCache == nil or trainManagerEntry.leavingTrainRearCarriageCache.speedPositive ~= (leavingTrainSpeed > 0) then
            -- No cache entry or cache exists, but needs updating.
            leavingTrainRearCarriage = TrainManagerFuncs.GetRearCarriageOfLeavingTrain(trainManagerEntry.leavingTrain, trainManagerEntry.leavingTrainPushingLoco)
            trainManagerEntry.leavingTrainRearCarriageCache = {
                speedPositive = leavingTrainSpeed > 0,
                carriage = leavingTrainRearCarriage
            }
        else
            -- Use the cache rear carriage.
            leavingTrainRearCarriage = trainManagerEntry.leavingTrainRearCarriageCache.carriage
        end
        if Utils.GetDistanceSingleAxis(leavingTrainRearCarriage.position, trainManagerEntry.aboveExitPortalEndSignal.entity.position, trainManagerEntry.tunnel.railAlignmentAxis) > 20 then
            ...
            local scheduleBeforeCarriageAttachment, isManualBeforeCarriageAttachment, targetTrainStopBeforeCarriageAttachment, leavingAbsoluteSpeedBeforeCarriageAttachment = trainManagerEntry.leavingTrain.schedule, trainManagerEntry.leavingTrain.manual_mode, trainManagerEntry.leavingTrain.path_end_stop, math.abs(leavingTrainSpeed)
            ...
            TrainManagerFuncs.TrainSetSchedule(trainManagerEntry.leavingTrain, scheduleBeforeCarriageAttachment, isManualBeforeCarriageAttachment, targetTrainStopBeforeCarriageAttachment)
            ...
            TrainManager.Remote_TunnelUsageChanged(trainManagerEntry.id, TrainManager.TunnelUsageAction.leavingCarriageAdded)
            trainManagerEntry.leavingTrainRearCarriageCache = {
                speedPositive = trainManagerEntry.leavingTrain.speed > 0,
                carriage = TrainManagerFuncs.GetRearCarriageOfLeavingTrain(trainManagerEntry.leavingTrain, trainManagerEntry.leavingTrainPushingLoco)
            }
...
TrainManager.EnsureManagedTrainsFuel = function(trainManagerEntry, undergroundTrainSpeed)
    ...
    if undergroundTrainSpeed < trainManagerEntry.undergroundTrainOldAbsoluteSpeed and undergroundTrainSpeed < 0.1 and undergroundTrain.state == defines.train_state.on_the_path then
        local leadLocoBurner = trainManagerEntry.undergroundTrainAForwardsLocoBurnerCache
        if trainManagerEntry.undergroundTrainAForwardsLocoBurnerCache == nil then
            -- Not cached so obtain and then store it.
            local leadLoco
            if trainManagerEntry.undergroundTrainForwards then
            ...
            trainManagerEntry.undergroundTrainAForwardsLocoCache, trainManagerEntry.undergroundTrainAForwardsLocoBurnerCache = leadLoco, leadLocoBurner
        end
    ...
    trainManagerEntry.undergroundTrainOldAbsoluteSpeed = undergroundTrainSpeed
end

TrainManager.GetEnteringTrainLeadCarriageCache = function(trainManagerEntry, enteringTrain, enteringTrainForwards)
    -- Returns the cached lead carriage and records if needed.
    if trainManagerEntry.enteringTrainLeadCarriageCache == nil or trainManagerEntry.enteringTrainLeadCarriageCache.enteringTrainForwards ~= enteringTrainForwards then
        -- No cache entry or cache exists, but needs updating.
        local enteringTrainLeadCarriage = TrainManagerFuncs.GetLeadingWagonOfTrain(enteringTrain, enteringTrainForwards)
        trainManagerEntry.enteringTrainLeadCarriageCache = {
            enteringTrainForwards = enteringTrainForwards,
            carriage = enteringTrainLeadCarriage
        }
        return enteringTrainLeadCarriage
    else
        -- Use the cache lead carriage.
        return trainManagerEntry.enteringTrainLeadCarriageCache.carriage
    end
end
...
TrainManager.SetAbsoluteTrainSpeed = function(trainManagerEntry, trainAttributeName, absoluteSpeed)
...
TrainManager.CreateUndergroundTrainObject = function(trainManagerEntry)
    ...
    local undergroundTrain = TrainManager.CopyEnteringTrainUnderground(trainManagerEntry, firstCarriagePosition)
    trainManagerEntry.undergroundTrain = undergroundTrain
    trainManagerEntry.undergroundTrainCarriageCount = #undergroundTrain.carriages
...
TrainManager.ReverseManagedTrainTunnelTrip = function(oldTrainManagerEntry)
    ...
    newTrainManagerEntry.undergroundTrainAForwardsLocoCache = nil -- Will be populated on first use if needed.
    newTrainManagerEntry.undergroundTrainAForwardsLocoBurnerCache = nil -- Will be populated on first use if needed.
    ...
    if oldTrainManagerEntry.enteringTrainState == EnteringTrainStates.entering then
        ...
        TrainManagerFuncs.TrainSetSchedule(newTrainManagerEntry.leavingTrain, newTrainSchedule, false, newTrainManagerEntry.targetTrainStop, false)
    ...
    elseif oldTrainManagerEntry.enteringTrainState == EnteringTrainStates.finished then
        ...
        newTrainManagerEntry.dummyTrain = TrainManagerFuncs.CreateDummyTrain(newTrainManagerEntry.aboveExitPortal.entity, newTrainSchedule, newTrainManagerEntry.targetTrainStop, false)
    ...
    newTrainManagerEntry.undergroundTrainOldAbsoluteSpeed = 0
    ...
    local targetStation = newTrainManagerEntry.targetTrainStop
