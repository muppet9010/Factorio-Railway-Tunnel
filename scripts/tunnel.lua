...
Tunnel.CheckProcessTunnelPortalComplete = function(startingTunnelPortal)
    local centerPos, force, lastUser, directionValue, directionName, aboveSurface = startingTunnelPortal.position, startingTunnelPortal.force, startingTunnelPortal.last_user, startingTunnelPortal.direction, Utils.DirectionValueToName(startingTunnelPortal.direction), startingTunnelPortal.surface
    ...
    while continueChecking do
        ...
        if #connectedTunnelEntities == 0 then
            ...
            if TunnelSegmentEntityNames[connectedTunnelEntity.name] then
                if connectedTunnelEntity.direction == startingTunnelPortal.direction or connectedTunnelEntity.direction == Utils.LoopIntValueWithinRange(startingTunnelPortal.direction + 4, 0, 7) then
            ...
            elseif TunnelPortalEntityNames[connectedTunnelEntity.name] then
                continueChecking = false
                if connectedTunnelEntity.direction == Utils.LoopIntValueWithinRange(startingTunnelPortal.direction + 4, 0, 7) then
                    completeTunnel = true
...
Tunnel.TunnelCompleted = function(tunnelPortals, tunnelSegments)
    NEEDS FIXING UP
    for portalEntity in pairs(tunnelPortals) do
        local portal = global.tunnel.portals[portalEntity.unit_number]
        local centerPos, force, lastUser, directionValue, directionName, aboveSurface = portalEntity.position, portalEntity.force, portalEntity.last_user, portalEntity.direction, Utils.DirectionValueToName(portalEntity.direction), portalEntity.surface
        local orientation = directionValue / 8
